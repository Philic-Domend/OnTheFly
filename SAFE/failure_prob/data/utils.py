...
class TimestepRolloutDataset(Dataset):
    '''
    [新增类] A PyTorch Dataset for rollout data with timestep-level labels.
    '''
    def __init__(self, cfg: Config, rollouts: list[Rollout], device=None):
        self.cfg = cfg
        self.rollouts = rollouts
        self.length = len(rollouts)
        
        self.device = device or ("cuda" if cfg.dataset.load_to_cuda else "cpu")

        self.mp4_paths = [r.mp4_path for r in rollouts]
        
        # 填充批次
        features, masks, rollout_labels, step_labels, action_vectors = pad_rollout_batch_with_timestep_labels(rollouts, self.device)
        
        self.features = features
        self.masks = masks
        self.rollout_labels = rollout_labels
        self.step_labels = step_labels
        self.action_vectors = action_vectors
        
        # 统计
        n_with_timestep = sum(r.step_labels is not None for r in rollouts)
        print(f"TimestepRolloutDataset: {n_with_timestep}/{self.length} with timestep labels")

        # 输出处理后的每条rollout信息
        print("\nProcessed rollout info:")
        for i in range(self.length):
            print(f"\n[{i}] {self.mp4_paths[i]}")
            print(f"  features shape: {list(self.features[i].shape)}")
            print(f"  step_labels shape: {list(self.step_labels[i].shape)}")
            
            mask = self.masks[i].bool()
            valid_step_labels = self.step_labels[i]
            
            print(f"step_labels values: {valid_step_labels.tolist()}")
    ...
    def __getitem__(self, idx):
        data = {
            ...
            'rollout_labels': self.rollout_labels[idx],
    ...
    def get_mp4_paths(self):
        return self.mp4_paths
    

def pad_rollout_batch_with_timestep_labels(rollouts: list[Rollout], device=None):
    '''
    [新增方法] 填充批次，包含时间步级标签
    '''
    batch_size = len(rollouts)
    max_len = max(r.hidden_states.shape[0] for r in rollouts)
    hidden_dim = rollouts[0].hidden_states.shape[-1]
    
    # 初始化
    features = torch.zeros(batch_size, max_len, hidden_dim, device=device)
    step_labels = torch.full((batch_size, max_len), -1.0, device=device)  # -1表示填充
    masks = torch.zeros(batch_size, max_len, device=device)
    
    # 填充
    for i, r in enumerate(rollouts):
        seq_len = r.hidden_states.shape[0]
        features[i, :seq_len] = r.hidden_states.to(device)
        masks[i, :seq_len] = 1
        
        if r.step_labels is not None:
            step_labels[i, :seq_len] = r.step_labels.to(device)
        else:
            step_labels[i, :seq_len] = r.episode_success
            print("ERROR ERROR ERROR ERROR ERROR ERROR ERROR ERROR ERROR ERROR ERROR ERROR ERROR ERROR ERROR ERROR")
            print("ERROR ERROR ERROR ERROR ERROR ERROR ERROR ERROR ERROR ERROR ERROR ERROR ERROR ERROR ERROR ERROR")
            print("ERROR ERROR ERROR ERROR ERROR ERROR ERROR ERROR ERROR ERROR ERROR ERROR ERROR ERROR ERROR ERROR")
    
    # rollout级标签
    rollout_labels = torch.tensor([r.episode_success for r in rollouts], device=device)
    
    # 动作向量
    if rollouts[0].action_vectors is not None:
        action_dim = rollouts[0].action_vectors.shape[-1]
        action_vectors = torch.zeros(batch_size, max_len, action_dim, device=device)
        for i, r in enumerate(rollouts):
            seq_len = r.action_vectors.shape[0]
            action_vectors[i, :seq_len] = r.action_vectors.to(device)
    else:
        action_vectors = None
    
    return features, masks, rollout_labels, step_labels, action_vectors


class PlusRolloutDataset(Dataset):
    '''
    [新增类] A PyTorch Dataset for rollout data with timestep-level labels.
    '''
    def __init__(self, cfg: Config, rollouts: list[Rollout], device=None):
        ...
        # 填充批次
        features, vl, masks, rollout_labels, step_labels, action_vectors = pad_rollout_batch_plus(rollouts, self.device)
        ...
        # 统计
        n_with_timestep = sum(r.step_labels is not None for r in rollouts)
        print(f"PlusRolloutDataset: {n_with_timestep}/{self.length} with timestep labels")
        ...
        for i in range(self.length):
            print(f"\n[{i}] {self.mp4_paths[i]}")
            print(f"  features shape: {list(self.features[i].shape)}")
            print(f"  vl shape: {list(self.vl[i].shape)}")
            print(f"  step_labels shape: {list(self.step_labels[i].shape)}")
...
def pad_rollout_batch_plus(rollouts: list[Rollout], device=None):
    '''
    [新增方法] 填充批次，包含时间步级标签
    '''
    batch_size = len(rollouts)
    max_len = MAX_LEN
    hidden_dim = rollouts[0].hidden_states.shape[-1]
    vl_dim = rollouts[0].vl_states.shape[-1]
    
    # 初始化
    features = torch.zeros(batch_size, max_len, hidden_dim, device=device)
    vl = torch.zeros(batch_size, max_len, vl_dim, device=device)
    step_labels = torch.full((batch_size, max_len), -1.0, device=device)  # -1表示填充
    masks = torch.zeros(batch_size, max_len, device=device)
    ...
    for i, r in enumerate(rollouts):
        seq_len = r.hidden_states.shape[0]
        features[i, :seq_len] = r.hidden_states.to(device)
        vl[i, :seq_len] = r.vl_states.to(device)
        masks[i, :seq_len] = 1
        
        if r.step_labels is not None:
            indices = torch.arange(0, seq_len * 5, 5)  # 0, 5, 10, ..., seq_len*5-5
            step_labels[i, :seq_len] = r.step_labels[indices].to(device)  # 形状: [seq_len, hidden_dim]
        else:
            step_labels[i, :seq_len] = r.episode_success
            print("ERROR ERROR ERROR ERROR ERROR ERROR ERROR ERROR ERROR ERROR ERROR ERROR ERROR ERROR ERROR ERROR")
    ...
    return features, vl, masks, rollout_labels, step_labels, action_vectors