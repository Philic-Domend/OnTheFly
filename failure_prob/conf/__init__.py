...
class DatasetConfig:
    ...
    data_paths: Optional[List[str]] = None # 新增！
    success_labels_paths: Optional[List[str]] = None # 新增！
    task_names: Optional[List[str]] = None # 新增！
...
@dataclass
class WinTransformerModelConfig(ModelConfig):
    name: str = "winTransformer"

    # ---- window / input ----
    n_history_steps: int = 3
    if_use_vl: bool = False
...
cs.store(
    group="model",
    name="winTransformer",
    node=WinTransformerModelConfig,
)
...
def process_cfg(cfg: Config):
    if cfg.dataset.data_path_prefix is not None:
        ...
        if isinstance(cfg.dataset.data_paths, omegaconf.listconfig.ListConfig):
            for i in range(len(cfg.dataset.data_paths)):
                cfg.dataset.data_paths[i] = cfg.dataset.data_path_prefix + cfg.dataset.data_paths[i]
        elif isinstance(cfg.dataset.data_paths, str):
            cfg.dataset.data_paths = cfg.dataset.data_path_prefix + cfg.dataset.data_paths
        else:
            raise ValueError(f"DatasetConfig: data_paths should be a string or a list of strings, but got {type(cfg.dataset.data_paths)}")
        print(f"DatasetConfig: set data_paths to {cfg.dataset.data_paths}")

        if isinstance(cfg.dataset.success_labels_paths, omegaconf.listconfig.ListConfig):
            for i in range(len(cfg.dataset.success_labels_paths)):
                cfg.dataset.success_labels_paths[i] = cfg.dataset.data_path_prefix + cfg.dataset.success_labels_paths[i]
        elif isinstance(cfg.dataset.success_labels_paths, str):
            cfg.dataset.success_labels_paths = cfg.dataset.data_path_prefix + cfg.dataset.success_labels_paths
        else:
            raise ValueError(f"DatasetConfig: success_labels_paths should be a string or a list of strings, but got {type(cfg.dataset.success_labels_paths)}")
        print(f"DatasetConfig: set success_labels_paths to {cfg.dataset.success_labels_paths}")
