...
def main(cfg: Config) -> None:
    ...
    for seed in seeds:
        ...
        # --- 修正后的 [Example rollout attributes] 输出 ---
        if "train" in rollouts_by_split_name and len(rollouts_by_split_name["train"]) > 0:
            example_r = rollouts_by_split_name["train"][0]
            print("\n" + "="*50)
            print(f">>> [Example rollout attributes] (Split: train, Index: 0)")
            
            for attr_name in dir(example_r):
                if not attr_name.startswith('_'):
                    attr_val = getattr(example_r, attr_name)
                    if callable(attr_val):
                        continue
                        
                    if hasattr(attr_val, 'shape'):
                        # 先转成 str，再进行宽度对齐
                        shape_str = str(list(attr_val.shape))
                        print(f"  - {attr_name:<18} : Shape {shape_str:<20} | Dtype: {attr_val.dtype}")
                    else:
                        val_str = str(attr_val)
                        print(f"  - {attr_name:<18} : Value {val_str:<20} | Type: {type(attr_val).__name__}")
            print("="*50 + "\n")
        ...
        for epoch in pbar:
            ...
            if epoch % cfg.train.roc_every == 0 or epoch == n_epochs - 1:
                eval_logs = eval_model_and_log(
                    ...
                    rollouts_by_split_name, 
                    dataloader_by_split_name,
                    EVAL_TIME_QUANTILES[cfg.dataset.name], # [0.25, 0.5, 0.75, 1.0],
