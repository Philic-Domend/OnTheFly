        ...
        for alpha in alphas:
            ...
            for calib_label in ['pos', 'neg']:
                if calib_label == 'pos':
                    ...
                    ### !
                    if alpha == 0.15 and eval_time == "by final end":
                        print("yes!!!!!")
                    ### !
    ...
    for calib_on in ['neg']:
        ...
        for eval_time in ['by final end', 'by earliest stop']:
            for alpha in alphas:
                ...
                if eval_time == "by final end" and alpha == 0.15:
                    print(f"test_labels_all: {test_labels_all.shape}, 数值:\n {test_labels_all}\n")
                    print(f"has_detection形状: {has_detection.shape}, 数值:\n {has_detection}\n")
                    print(f"relative_detection_times形状: {relative_detection_times.shape}, 数值:\n {relative_detection_times}\n")
...
def eval_functional_conformal_timestep(
    step_scores_by_split_name: dict[str, np.ndarray],
    step_labels_by_split_name: dict[str, np.ndarray],
    method_name: str,
    calib_split_names: list[str] = ["val_seen"],
    test_split_names: list[str] = ["val_unseen"],
):
    ...
    # 收集数据
    cal_scores = np.concatenate([step_scores_by_split_name[s] for s in calib_split_names])
    cal_labels = np.concatenate([step_labels_by_split_name[s] for s in calib_split_names])
    test_scores = np.concatenate([step_scores_by_split_name[s] for s in test_split_names])
    test_labels = np.concatenate([step_labels_by_split_name[s] for s in test_split_names])
    
    # 只在校准成功的时间步上
    cal_success_scores = cal_scores[cal_labels == 1]
    
    if len(cal_success_scores) < 2:
        print(f"Warning: Not enough success timesteps for calibration ({len(cal_success_scores)})")
        return pd.DataFrame(logs), {}
    
    # 分割校准数据
    np.random.shuffle(cal_success_scores)
    n_cal_1 = int(len(cal_success_scores) * 0.3)
    cal_scores_1 = cal_success_scores[:n_cal_1]
    cal_scores_2 = cal_success_scores[n_cal_1:]
    
    for alpha in alphas:
        # 计算分位数阈值
        threshold = np.percentile(cal_scores_1, (1 - alpha) * 100)
        
        # 检测：分数超过阈值
        detection = test_scores >= threshold
        
        # 计算指标
        tp = (detection & (test_labels == 1)).sum()
        tn = (~detection & (test_labels == 0)).sum()
        fp = (detection & (test_labels == 0)).sum()
        fn = (~detection & (test_labels == 1)).sum()
