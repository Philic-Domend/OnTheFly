...
class Pi0(_model.BaseModel):
    ...
    def sample_actions(
        ...
        *,
        ...
        vl_states, kv_cache = self.PaliGemma.llm([prefix_tokens, None], mask=prefix_attn_mask, positions=positions)
        # vl_states: [1, 816, 2048]
        vl_states = vl_states[0]
        # vl_states: [816, 2048]
        ...
        # 简洁版本
        # print(f"xs - type: {type(xs)}, shape: {xs.shape}, dtype: {xs.dtype}")
        # print(f"aux_accum - type: {type(aux_accum)}, keys: {list(aux_accum.keys())}")

        '''
        for key, value in aux_accum.items():
            if hasattr(value, 'shape'):
                print(f"aux_accum['{key}'] - shape: {value.shape}, dtype: {value.dtype}")
            else:
                print(f"aux_accum['{key}'] - type: {type(value)}")
        '''
