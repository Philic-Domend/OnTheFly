...
class PolicyRecorder(_base_policy.BasePolicy):
    ...
    @override
    def infer(self, obs: dict) -> dict:  # type: ignore[misc]
        def print_results_info(results, prefix=""):
            """
            打印字典或 jax.tree 的内容，包括每个字段的 shape 和 dtype。
            递归处理嵌套字典。
            """
            if isinstance(results, dict):
                for k, v in results.items():
                    print_results_info(v, prefix=f"{prefix}{k}")
            elif isinstance(results, (np.ndarray, jnp.ndarray)):
                print(f"{prefix}: type={type(results)}, shape={results.shape}, dtype={results.dtype}")
            elif isinstance(results, (list, tuple)):
                print(f"{prefix}: type={type(results)}, length={len(results)}")
                for i, item in enumerate(results):
                    print_results_info(item, prefix=f"{prefix}[{i}]")
            else:
                print(f"{prefix}: type={type(results)}, value={results}")
        
        results_multi = self._policy._infer_maybe_multi(obs)
        # print(f"self._policy: {self._policy}")
        # print_results_info(results_multi)
